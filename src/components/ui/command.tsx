...
const CommandInput = React.forwardRef<React.ElementRef<typeof CommandPrimitive.Input>, React.ComponentPropsWithoutRef<typeof CommandPrimitive.Input>>(({ className, ...props }, ref) => (
  <div
    className="flex items-center gap-2 border-b border-border/50 bg-muted/10 px-3 py-2 transition-[background-color,border-color] focus-within:border-border/60 focus-within:bg-muted/20"
    cmdk-input-wrapper=""
  >
    <MagnifyingGlassIcon className="h-4 w-4 shrink-0 text-muted-foreground/80" />
    <CommandPrimitive.Input
      ref={ref}
      className={cn(
        "flex h-9 min-w-0 flex-1 border-0 bg-transparent py-0 text-sm text-foreground shadow-none outline-none ring-0",
...
const CommandItem = React.forwardRef<React.ElementRef<typeof CommandPrimitive.Item>, React.ComponentPropsWithoutRef<typeof CommandPrimitive.Item>>(({ className, ...props }, ref) => (
  <CommandPrimitive.Item
    ref={ref}
    className={cn(
      "group relative flex cursor-default select-none items-center rounded-sm px-2 py-1.5 text-sm outline-none data-[disabled=true]:pointer-events-none data-[selected=true]:bg-muted/70 data-[selected=true]:text-foreground data-[disabled=true]:opacity-50",
      className
    )}
    {...props}
  />
